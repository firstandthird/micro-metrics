...
tap.test('can use the report method to get an aggregate list of metrics grouped by values for a given tag in csv format', (t) => {
  async.autoInject({
    init(done) {
      setup.withRapptor({}, [{
        type: 'BankAccount',
...
tap.test('can pass last=0 to unique to override 30 day default', (t) => {
  async.autoInject({
    init(done) {
      setup.withRapptor({}, [{
        type: 'BankAccount',
